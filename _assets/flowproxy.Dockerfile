# flowproxy, built arch-agnostically. the flowproxy repo's own
...
#
# Two copies exist on purpose until the upstream fix (the flowproxy repo
...
COPY --from=src . .
# proposed upstream (the flowproxy repo fix/dockerfile-multiarch): --chain-id / CHAIN_ID

...
// what production runs: flashbots-images origin/trunk/buildernet (dc854ad,
// 2026-09-16) flowproxy-downloader.service FLOWPROXY_REPO=flowproxy-private
...
// into the image).
export const FLOWPROXY_REPO = "https://github.com/flashbots/flowproxy-private";
...
// fixes proposed on flowproxy-private (fix/dockerfile-multiarch):
...
// buildernet-configs base.libsonnet); here the same ports are the pod's,
...
// derives it from Builder Hub per node; a single-node devnet trusts itself.
...
export function buildContainer(def: ContainerDef, ctx: Ctx): ContainerResult {
  ...
  // minus ClickHouse (no sink in a dev env) and Builder Hub (STATIC_PEERS is

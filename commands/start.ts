...
import { resolveInput, type TargetOverride, upManifest, upProject, upRecipeFile } from "./up.ts";
...
export const command = new Command()
  ...
  .action(async (opts, arg?: string) => {
    ...
    const runOpts = { scripts: opts.script, options: parseOpts(opts.opt), summarize: true };
    let out;
    if (opts.manifest) {
      out = await upManifest(opts.manifest, override, runOpts);
    } else {
      ...
      out = await upRecipeFile(input.ref, override, runOpts);
    }
